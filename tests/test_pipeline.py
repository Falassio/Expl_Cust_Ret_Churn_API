...
def test_synthetic_data_generation():
    ...
    assert all(col in df.columns for col in ["tenure_months", "monthly_charges", "total_tickets", "contract_type", "payment_method"])
...
def test_train_and_evaluate(tmp_path):
    # Run training with save_artifacts=False to verify logic
...
def test_explainability_computation():
    ...
    explanations = compute_instance_explanations(explainer, X_trans, feature_names, top_k=3)
    
    assert len(explanations) == 3
    assert all("feature" in item and "impact_score" in item and "description" in item for item in explanations)
